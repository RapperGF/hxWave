...
LibVLC::LibVLC(void)
{
	...
	libVlcInstance = libvlc_new(3, argv); // NOW WHO'S GONNA STOP MEEEEEEEE!? 
}
...
void LibVLC::playFile(const char *path, bool loop, bool haccelerated, float pitch)
{
	...
	// TODO: add byte streaming.
	//libVlcMediaItem = libvlc_media_new_callbacks(libVlcInstance, path);
	libVlcMediaPlayer = libvlc_media_player_new_from_media(libVlcMediaItem);
	//libvlc_media_player_set_nsobject(libVlcMediaPlayer, nullptr );
	...
	// libvlc doesnt support audio filters.
	//libvlc_media_add_option(libVlcMediaItem, ":pitch-shift=2.0");
	//ibvlc_media_add_option(libVlcMediaItem, ":audio-filter=scaletempo_pitch");
...
/*\void LibVLC::setRate(float rate)
{
	libvlc_media_player_set_rate(libVlcMediaPlayer, rate);
}

float LibVLC::playbackRate()
{
	if (!libVlcMediaPlayer)
	return -1;

	return libvlc_media_player_get_rate(libVlcMediaPlayer);
}*/
